...
frontend/
  src/
    ...
    routes/
      ...
      MainView.tsx
      EditSummaryView.tsx
      CourseView.tsx          per-course overview view: one "Generate" button per extractor (+ an open-PDF button when its {slug}.pdf exists) + a "Generate All" button; SSE-refreshed status (no per-phase UI, no file links)
    components/
      sidebar/
