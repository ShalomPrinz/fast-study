  ...
  src/
    ...
    routes/
      ...
      course/
        ...
        ExtractorRow.tsx           1 prop `extractor`: one extractor row. Reads useCourseOverview(); owns local `expanded` (default false — rows start collapsed) + the whole-extractor regenerate modal + (until B5) the per-step regenerate modal; provides ExtractorContext. Renders <ExtractorHeader/> + the inline per-phase steps JSX (B5 extracts). Own handleGenerate → toastInitResult
