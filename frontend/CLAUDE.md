  ...
  src/
    ...
    features/
      ...
      downloads/            the Downloads page (auto-downloader integration), a main-layout view peer to CourseView — no sidebar mode
        ...
        utils/
          nameSuggestion.ts     suggestItemName(title,kind,courses,course): first integer in the title → "Lecture N"/"Recitation N", plus a single adjacent Latin letter as a decimal sub-number ("11a" → "Lecture 11.1", a=1…z=26; "11 A"/"11AB" → just 11); no number → suggestName(courses,course,kind) fallback. isDownloaded(name,kind,courses,course) is the single already-downloaded rule (exact match in the course's lectures/recitations); suggestedDownload(title,kind,courses,course) → {name, alreadyDownloaded} is what the bulk path uses so it can't diverge from a row
