...
frontend/
  src/
    ...
    routes/
      ...
      MainView.tsx
      EditSummaryView.tsx
      CourseView.tsx          per-course overview view: one "Generate" button per extractor — swapped for a ↺ re-generate button (opens a ConfirmModal listing {slug}.txt/.md/.pdf) once its {slug}.pdf exists — (+ an open-PDF button then) + a "Generate All" button; SSE-refreshed status (no per-phase UI, no file links)
    components/
      sidebar/
