...
function failure(state, code, params) {
  return { state, code, params };
}
  ...
  return new Promise((resolve) => {
    execFile(
      ...
      (err) => {
        ...
        if (err.code === 'ENOENT') return resolve(failure('missing', 'tool_missing', { tool: name }));
        // execFile reports a timeout as the signal it killed the child with, not as a code.
        const seconds = VERSION_TIMEOUT_MS / 1000;
        if (err.killed)
          return resolve(
            failure(`timed out after ${seconds}s`, 'tool_probe_timeout', { tool: name, seconds }),
          );
        if (typeof err.code === 'number')
          return resolve(
            // snake_case on the wire in both halves: the code namespace spans all four services.
            failure(`exited ${err.code}`, 'tool_probe_exit', { tool: name, exit_code: err.code }),
          );
        resolve(
          failure(`unusable: ${err.message}`, 'tool_unusable', { tool: name, detail: err.message }),
        );
      },
    );
  });
...
// Every name mapped to 'ok' or a {state, code, params} record saying why it is not usable. Never
