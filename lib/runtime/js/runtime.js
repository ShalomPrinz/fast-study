...
export function statePath(...parts) {
  ...
  // `||`, not `??`: an empty launch variable reads as unset in both halves — Python's `or` does the
  // same — so a blank FASTSTUDY_STATE_DIR cannot turn this into a relative join under the cwd.
  const root =
