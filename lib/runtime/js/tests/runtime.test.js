...
const SECRET = 's3cr3t';
// Same length as SECRET on purpose: a length mismatch never reaches timingSafeEqual at all.
const WRONG = 'wrong!';
...
test('no credential at all is rejected', async () => {
  process.env.FASTSTUDY_SECRET = SECRET;
  // req.get() returns undefined here; a 500 would mean the typeof guard was dropped and
  // Buffer.from threw instead.
  assert.equal((await get('/thing')).status, 401);
});

test('a same-length wrong secret is rejected, as the header and as ?secret=', async () => {
  process.env.FASTSTUDY_SECRET = SECRET;
  // The only cases that reach timingSafeEqual at all — every other rejection stops at the length
  // pre-check, so degrading secretMatches to a length comparison would leave them green.
  assert.equal((await get('/thing', { headers: { [HEADER]: WRONG } })).status, 401);
  assert.equal((await get(`/thing?secret=${WRONG}`)).status, 401);
});
