...
function startJob(res, downloader, input, { course, lecture, kind, ref, fromCache }) {
  const jobId = createJob({ course, lecture, kind, tool: downloader.tool, ref, fromCache });
  runDownloadJob(downloader, input, { course, lecture, kind, jobId, ref, fromCache });
  res.json({ status: 'Downloading in background...', target: `${course}/${lecture}`, jobId });
}

router.post('/download', (req, res) => {
  const { url, headers, course, lecture, kind = 'lecture', ref, fromCache } = req.body ?? {};
