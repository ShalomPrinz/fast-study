...
import { cancelRun, getRun, listRuns, resumeRun, startRun, UNKNOWN_RUN } from '../runs.js';
...
function targetError(target) {
  if (typeof target?.ref !== 'string' || !target.ref)
    return invalidRequest('ref', 'each target needs a valid ref');
  if (!storedName(target.name))
    return invalidRequest('name', 'each target needs a name with a legal character');
...
router.post('/download-section', (req, res) => {
  ...
  if (typeof sectionId !== 'string' || !sectionId) {
    return res.status(400).json(invalidRequest('sectionId', 'valid sectionId required'));
  }
  const storedCourse = storedName(course);
  if (!storedCourse) {
    return res
      .status(400)
      .json(invalidRequest('course', 'course with a legal character is required'));
  }
  if (!Array.isArray(targets) || !targets.length) {
    return res.status(400).json(invalidRequest('targets', 'targets must be a non-empty array'));
  }
...
router.post('/runs/:id/resume', (req, res) => {
  if (!getRun(req.params.id)) return res.status(404).json(UNKNOWN_RUN);
