...
own: it captures a video (curl header-replay or yt-dlp) or receives a PDF, then hands the bytes to the **database service**, which writes them under
`DATA_ROOT`. 
...
| `DOWNLOADER_EXTENSION_ID`| `kebkiehjoihdofnobkbifjcihnifibdo`   | the only allowed CORS origin     |
