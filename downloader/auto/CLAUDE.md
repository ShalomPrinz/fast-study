...
`yt-dlp` is spawned through `toolPath` from `@faststudy/tools` — `FASTSTUDY_BIN_DIR` set means an absolute path into the shipped binaries, unset means PATH — and probed once at startup, reported on `/health` as `tools`. Only playlist expansion uses it, and only with `--flat-playlist`, which never touches YouTube's player script, so it carries none of `server/`'s JS-runtime flags.
...
| `GET /prereqs/browser`  | —                                                   | `{ available, channel, browser, detail }` — is a Chromium-family browser installed                                       |
...
`GET /prereqs/browser` answers **200 either way** — "no browser" is an answer, not a failure. `available:true` carries `channel` (`'chrome'`|`'msedge'`, the Playwright channel) and `browser` (its display name, e.g. `Microsoft Edge`, so the settings screen names what it found); `available:false` carries both as `null` and every path tried in `detail`, which is a one-line human string in both cases. A browser gates the whole download surface and none of the pipeline. A negative is never cached, so re-checking after the user installs one needs no restart (`docs/SESSIONS.md`).

`401 {status:'reconnect'}` = the Moodle WS token is missing or a call returned `invalidtoken`. `422 {status:'unsupported'}` = the source genuinely can't be handled: a `url` module target no download-time probe could turn into a file (the Drive probe resolves the real filename, the generic one reads the URL path then `Content-Disposition`/`Content-Type`, and both route video → yt-dlp, `.pdf` → material; anything else 422s naming what it turned out to be, and a Drive file that isn't shared "anyone with the link" yields no filename and 422s naming the sharing cause and the URL). Every unsupported verdict is memoized under its probe key, so they re-throw off the cache and stamp `resolvedMedia:'unsupported'` on `/list`; `forceCapture` re-probes. `409 {status:'passcode', reason, course, name}` = zoom passcode `missing` (none stored) or `incorrect` (stored one won't clear the gate); save one via `POST /zoom/passcode` and retry. `503 {status:'blocked', message}` = the Moodle site served a bot-protection challenge (HTML or a redirect) instead of a web-service answer, so the call could not run at all; transient, unrelated to the token, and not retried here (`docs/MOODLE.md`).
