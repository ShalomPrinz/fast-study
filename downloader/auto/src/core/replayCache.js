...
function keyOf(course, lecture, kind) {
  return JSON.stringify([course, lecture, kind]);
}
...
export function cacheCap(course, lecture, kind, cap, ref) {
  entries.set(keyOf(course, lecture, kind), { cap, ref });
}

// Cached cap for one target, or null.
export function getCap(course, lecture, kind) {
  return entries.get(keyOf(course, lecture, kind)) ?? null;
}
