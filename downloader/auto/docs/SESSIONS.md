...
The service holds long-lived browsers (the ~1–2s launch + auth-context build is paid once), not one-per-request. Model in `src/browser/browserSession.js`.
...
|---|---|---|---|
| `'plain'` — `/list`, `/list/expand`, videostream (`browser/browserLaunch.js`, plain bundled Chromium, no stealth) | headless | none | no |
