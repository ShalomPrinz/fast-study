...
| ------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------ | ------------------ |
| `'plain'` — videostream `.mp4` sniff only (`/list` and `/list/expand` need no browser; `browser/browserLaunch.js`, no stealth) | system Chrome `channel:'chrome'` → bundled Chromium on launch failure                                                                                                        | none                                       | no                 |
| `'zoom'` — `ZoomExtractor` capture (`browser/zoomBrowser.js`)                                                                  | system Chrome `channel:'chrome'` + puppeteer-extra stealth + `--disable-blink-features=AutomationControlled` + `ignoreDefaultArgs:['--enable-automation']`, `headless:false` | Xvfb (Linux) / off-screen window (Windows) | no                 |
| One-time token grab (`MoodleToken.connect()`, `launchBrowser` headed)                                                          | system Chrome `channel:'chrome'` → bundled Chromium on launch failure                                                                                                        | real desktop                               | **yes — MFA once** |

System Chrome is a hard prerequisite of every row above — without it the plain profile falls back to a bundled Chromium that Entra SSO tends to flag, and zoom capture is impossible.
