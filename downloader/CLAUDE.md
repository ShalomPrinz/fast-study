...
- **Never make a live request to `lemida.biu.ac.il` (or any university host) to verify a change.** Exercise the code against fixtures and a stubbed `globalThis.fetch` in `auto/test/*.test.js`, plus curls against the local services. Traffic that looks automated trips the Radware block for minutes, which then breaks the user's own real downloads (`auto/docs/MOODLE.md`).
