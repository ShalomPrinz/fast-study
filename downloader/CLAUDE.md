...
**Auth** (`src/auth/MicrosoftAuth.js`) is split for a UI trigger: `connect(entryUrl)` opens the headed login and returns immediately; `complete()` persists `storageState` and closes the headed browser; `status()` is a cheap probe (state file exists + cookie-expiry heuristic, no browser launch — the real redirect-to-login check happens on `/list`). The cookie heuristic can't see a server-side session kill, so when a **runtime** `/list` or `/download-item` navigation actually bounces to the login/enrol gate the handler calls `auth.markExpired()` on the cached instance — the server becomes the source of truth and the next `/auth/status` reports `expired:true` until the next successful `complete()` clears the flag. This relies on the per-university instance staying cached in `authInstances` (never evict it). The CLI's terminal-Enter path (`getAuthState`) reuses the same `connect`/`complete`. One auth instance is cached **per university** so `connect`→`complete` share the in-memory headed browser.
