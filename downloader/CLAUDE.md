...
`auto/` is a **separate package** (its own `node_modules`) so Playwright never leaks into the dependency-free `server/`. Its single entry point is the HTTP service, over one core (`src/core.js`: `listRecordings` + `downloadRecording`):

- **HTTP service** — `src/server.js` (`npm start`), Express on **port 3053** (`AUTODL_PORT`). Browser-facing: CORS allows the Vite origin `http://localhost:5173` (unlike `server/server.js`, locked to the extension ID). Reads the repo-root `.env` (via `src/config.js`) for `SERVER_URL` (default `http://localhost:3052`) — the `server/` base it POSTs downloads to.

**Discovery sources.** `listRecordings` first calls `expandTiles` (in `moodleCourse.js`): BIU runs Moodle **format_tiles**, which injects each section's body (activities AND summary) into the DOM only when its tile is clicked — so an unexpanded course parses as empty ("No recordings found"). `expandTiles` clicks every `#tile-N` / `.tile[data-section]` and waits for that section's content (no-op on non-tiles formats; per-tile failures swallowed). It then merges two DOM parsers: `parseMoodleCourse` (walks `li.activity` module cards → `videostream`/`url` extractors) **and** `parseZoomSections` (scans `li.section .summary` for `zoom.us/rec/share` links — `<a href>` or bare text — pairing each with its preceding `הרצאה מספר N` label). Only the `url` extractor (`YoutubePlaylistExtractor#canHandle`) is keyword-gated — `isRecording(sectionName, title)` in `moodleCourse.js` matches an allow-list (`RECORDING_KEYWORDS`: הקלטות/הרצאות/תרגולים/…/recording/lecture, case-insensitively) over the activity title AND its section/tile heading. WHY only `url`: a Moodle `url` module is an opaque off-site link (a YouTube playlist, but equally a syllabus, reading, or Drive folder), so unfiltered every `url` got optimistically surfaced as an expandable "recording" and blew up on expand; `videostream` (matched by module type — always an in-site video) and `zoom` (synthetic `modType:'zoom'` minted only for paragraphs carrying a real `zoom.us/rec/share` link) are already unambiguous. The latter feeds the `ZoomExtractor` (`strategy:'zoom'`, synthetic `modType:'zoom'`), whose `captureVideo` clears the passcode gate and sniffs the `.mp4` like `videostream`, but returns **1-or-2** captures: a zoom share can hold a before/after-break pair, split into `Lecture N.1`/`Lecture N.2` when the player's `.vjs-multiple-clip-control` reports "Total N Recordings" (N>1) and a distinct second `.mp4` is captured via the "next clip" control (`splitName` in `naming.js`). The passcode is **not** scraped or carried in the `ref` — every BIU share uses the single hardcoded `ZOOM_PASSWORD` (`config.js`), filled at the `#passcode` gate.

**Persistent browsers + mutex (per-extractor DI).** The browser is chosen **per extractor** and injected. `VideoExtractor.browserProfile` (default `'plain'`; `ZoomExtractor` overrides to `'zoom'`) names the profile; `BrowserSession` takes an injected launcher fn (not a hardcoded launch); `getSession(profile)` in `src/browserSession.js` lazily builds and keeps **one long-lived browser+context+page per profile** (each with its own idle timer), built from the stored `storageState`. Sessions are **not** closed between requests or when switching course — switching course is just `goto()`. All page ops run through `withLock(fn)`, a small async mutex that serializes only the quick navigate+sniff; the heavy download runs afterward in `server/server.js`, so parallel downloads still overlap end-to-end. Re-auth calls `rebuildOpenSessions(newState)` (fresh cookies on every open session, same browser processes). An idle-timeout (`IDLE_TIMEOUT_MS`, ~45 min) is only a leak-safety valve; a session re-opens lazily on its next call. `closeAllSessions()` (which also stops the managed Xvfb) runs on `/close` and `SIGINT`/`SIGTERM`; idle closes a single session.

The launch matrix (why per-profile — Zoom's recording player is the only path that needs the heavyweight browser):

| Profile / path | Browser | Display | Visible |
|---|---|---|---|
| `'plain'` — `/list`, `/list/expand`, videostream (`browserLaunch.js` `launchBrowser`, plain bundled Chromium, no stealth) | headless | none | no |
| `'zoom'` — `ZoomExtractor` capture (`zoomBrowser.js` `launchZoomBrowser`) | system Chrome `channel:'chrome'` + puppeteer-extra **stealth** (with `user-agent-override` evasion deleted) + `ignoreDefaultArgs:['--enable-automation']` + `--disable-blink-features=AutomationControlled`, `headless:false` | **managed Xvfb** virtual display | no (virtual) |
| Microsoft login (`MicrosoftAuth.connect()`, `launchBrowser` headed) | bundled Chromium → `channel:'chrome'` on launch failure | **real WSLg display** (inherited `DISPLAY`) | **yes — required for MFA** |

**Why the zoom profile.** Headless Chrome falls back to SwiftShader software rendering AND leaks a `HeadlessChrome` UA token, both of which the zoom recording player rejects. Headed **system Chrome under Xvfb** keeps the hardware D3D12 GPU renderer (WSL's `/dev/dxg` is reached independently of the X display) AND a clean `Chrome` UA, with **no visible window** (the display is virtual). Do NOT override the UA (it desyncs from Client-Hints → zoom flags it) or add `--use-angle=vulkan` (no HW Vulkan → SwiftShader). **Xvfb lifecycle** is managed in-process (`zoomBrowser.js`, Node `child_process`): spawned lazily on the first zoom launch with `-displayfd 3` (Xvfb picks a free display and writes its number back once ready — a deterministic readiness signal) and `-screen 0 1280x1024x24 -nolisten tcp`; killed by `stopXvfb()` on `closeAllSessions()` and a `process.once('exit')` safety net. The node process is **not** wrapped in `xvfb-run` — the login browser must stay on the real WSLg display.

**Auth** (`src/auth/MicrosoftAuth.js`) is split for a UI trigger: `connect(entryUrl)` opens the headed login and returns immediately; `complete()` persists `storageState` and closes the headed browser; `status()` is a cheap probe (state file exists + cookie-expiry heuristic, no browser launch — the real redirect-to-login check happens on `/list`). The cookie heuristic can't see a server-side session kill, so a **runtime** `/list` or `/download-item` navigation goes through `session.gotoAuthenticated(url, auth)`: if the nav bounces to the login/enrol gate it first attempts a **silent SSO recovery** — the short Moodle session can idle out while the persistent Microsoft/Entra (AAD) cookie is still alive, so it waits (bounded, `SILENT_REAUTH_TIMEOUT_MS` ~12s) for the page to auto-redirect back to a real page (no MFA); on success it re-saves the refreshed rolling cookies via `auth.saveState()` (best-effort) so the window keeps extending, and the operation proceeds. Only when recovery times out (the login/MFA form actually needs credentials) does the handler call `auth.markExpired()` on the cached instance — the server becomes the source of truth and the next `/auth/status` reports `expired:true` until the next successful `complete()` clears the flag. This relies on the per-university instance staying cached in `authInstances` (never evict it). One auth instance is cached **per university** so `connect`→`complete` share the in-memory headed browser.

**Auth gate** (in-memory, per university, mirrors `authInstances`). Browsing endpoints (`/list`, `/list/expand`, and the videostream branch of `/download-item` — NOT the zoom branch) `await authGate(uni)` before loading state / opening the browser. `/auth/connect` **closes** the gate; `/auth/complete` **opens** it *after* `complete()` + `rebuildOpenSessions()`. WHY: `open()` is a no-op when already open and skips `withLock`, so a `/list` fired during "finishing…" used to navigate on the stale pre-login context → login bounce → `markExpired()` flipped a **permanent** false "expired". Now that `/list` parks on the gate (async, zero-CPU) and runs on fresh cookies. The gate is released on **success** (`/auth/complete`, after the rebuild), on **abandon** (the headed login browser's `disconnected` event → `connect`'s `onCancel` → `openAuthGate`, immediate so a parked request wakes at once, re-checks `auth.status()` (still expired) and reconnects — retry is instant), or as a **backstop** after ~3 min (a `.unref()` timer, for the pathological case where the browser neither completes nor disconnects; it stays rare so a legitimately slow MFA login isn't cut early). `rebuildContext`/`open` are deliberately NOT wrapped in `withLock` (the gate already orders login-vs-browse; a shared lock across `/auth/complete` and browsing risks deadlock).
