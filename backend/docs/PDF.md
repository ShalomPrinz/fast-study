...
A **doubled** delimiter (markdown's way of putting a literal backtick in a span) is matched first and whole: matching the inner pair leaves the outer backticks in place, pandoc re-reads the rewritten span as code, and the `\LR{\texttt{…}}` prints literally. Markdown's one-space-of-padding strip is applied with it.
...
| `force_ltr_inline_code`         | Backtick spans → `\LR{\texttt{…}}`, skipping math. Enough for word order (a plain `\texttt` in an RTL paragraph reverses), since no `Verbatim` is involved. NOT enough for a comma-separated **number** list: digits are weak European Numbers and the comma is a neutral, so each comma jumps ahead of its number — an LRM (U+200E) after any comma preceding a digit pins it. Letters are strong L and never need this.                                                                                                                                                                                                              |
