...
backend/
  ...
  course/             per-COURSE logic (aggregates across a course's lectures — never belongs in pipeline/)
    ...
    runner.py            thin overview driver — try_run_generate schedules the sequential extract→analyze→topics→to_pdf run under ONE per-course lock (held across all phases); each phase runs only the extractors that declare it (`overview.EXTRACTORS_BY_SLUG[slug].phases`), phases with zero participants are skipped entirely (no advance/notify/work); owns run status, phase advancement, notify cadence, and the per-extractor loop (`_run_phase`) that isolates one extractor's failure as "error" without aborting the rest
