...
backend/
  ...
  pipeline/           per-LECTURE logic: pure functions, one module per step
    ...
    summarize.py         summarize(transcript_path, material_path=None) -> str (raises RuntimeError, or GeminiRateLimitError on a 429)
  ...
  services/
    ...
    llm_client.py     shared Gemini client — LLMClient(model): generate(contents)/upload_file/delete_file, GEMINI_API_KEY auth, SDK errors → RuntimeError, 429s → GeminiRateLimitError via parse_gemini_rate_limit (used by summarize.py + course/analyze.py)
...
- `transcribe.py` raises `TranscribeRateLimitError` carrying `{limit, used, requested, completed_chunks, total_chunks}` and writes partial state to disk; the next `/run/transcribe` call picks up from `transcript.partial.txt`. The transcribe endpoint round-trips the partial via the database service AND restores `audio.mp3`'s mtime from `transcript.partial.meta.json` — re-downloading audio.mp3 gives it a fresh mtime which would otherwise invalidate the resume meta and force a full restart.
- `timing/` records every pipeline operation's `(file_size, duration)` so the frontend can show calibrated ETAs.
- `runner.py` walks the tree looking for lectures that have `video.mp4` but lack `drive_url.txt` (`scan_pending`) and runs them sequentially through every missing step via `run_all(queue)`. It fires a `database/notify` SSE ping on each meaningful state change (step start/done, rate-limit start/wake, error, run start/complete) so the frontend can react without polling. On `rate_limited`, it sleeps the result's own `retry_after` when present (summarize sends `GEMINI_MINUTE_QUOTA_SLEEP_SECONDS`), else `RATE_LIMIT_SLEEP_SECONDS` (3600s — Groq's hourly ASR window, what transcribe uses), and retries the same step.
- **Gemini 429s split by quota kind.** `LLMClient.generate` parses the 429 body into `GeminiRateLimitError` (`{quota_id, quota_value, model, is_daily}`) instead of leaking the raw SDK JSON blob. The 429's `retryDelay` is never parsed — the quota kind already says everything actionable. A **per-minute** quota takes the normal `rate_limited` path, sleeping out its window; a **daily** quota (`quotaId` containing `PerDay`, or anything unrecognised — the safe default) is a plain error with a one-line message and no retry, because its `retryDelay` lies (says 59s, actually resets at midnight Pacific). The first such failure records the error and also sets the run-scoped `_summarize_blocked` flag, so every later lecture in the same `run_all` stops silently at `transcript.txt` — one error in the UI, not N. Audio+transcribe are Groq (separate quota) so the queue still gets fully transcribed, and `next_step` is pure file-existence so tomorrow's run resumes each lecture at summarize. Manual `/run/summarize` and `/pipeline` triggers ignore the flag (the user may have swapped keys).
