...
Course overview is one fire-and-forget trigger that runs THREE phases (extract → analyze → to_pdf) sequentially in the background under ONE per-course lock, returning `{"status": "started"|"busy"}` (`busy` when the lock is already held), or `{"status": "error", ...}` for an unknown extractor/course. `extractors` is an optional CSV of extractor **slugs** (default: all in `course/overview.py`'s `EXTRACTORS`). The slug (e.g. `exam-hints`) is the single identifier used on the wire, as the status-map key, and as the on-disk file stem — the display `title` (e.g. "Exam Hints") never appears in a filename or CSV. There is no per-phase endpoint; the frontend never sequences phases itself (mirrors `/run-all`).

- `POST /courses/{course}/overview/generate?extractors=exam-hints,pitfalls&from_phase=extract` — extract phase (scans every lecture's/recitation's `transcript.txt` tree-driven, writes one `{slug}.txt` snippet report per extractor to the course overview area), then analyze phase (per extractor reads its `{slug}.txt`, sends it to Gemini, writes `{slug}.md`; a missing `.txt` → `skipped` "no snippets file — run extract first"), then to_pdf phase (per extractor reads its `{slug}.md` and renders `{slug}.pdf` via `pipeline/to_pdf.py`'s `convert_to_pdf`; a missing `.md` → `skipped` "no analyzed markdown — run analyze first"). Optional `from_phase` (`extract` default | `analyze` | `to_pdf`; unknown value → `{"status": "error"}`) makes the run START from that phase and run through to_pdf — earlier phases are skipped, so their `{slug}.txt`/`.md` files are kept (never deleted). Starting from `analyze` needs `{slug}.txt`, from `to_pdf` needs `{slug}.md`; a missing input just yields the phase's usual `skipped`. One extractor's failure never aborts the others, and an extractor already in `error` from an earlier phase is left as-is and skipped by later phases (so the real failure survives to the final status, not masked as a downstream `skipped`). `running` stays `true` and the lock stays held across all three phases, so the UI shows one spinner with no false "done" flicker between phases.
- `GET /courses/{course}/overview/status` — `{"running", "phase": "extract"|"analyze"|"to_pdf"|null, "started_at", "extractors": {slug: {"status": "pending"|"running"|"done"|"skipped"|"error", "message"?}}}` (snake_case on the wire); `phase` advances extract→analyze→to_pdf as the run progresses; never-run course → `{"running": false, "phase": null, "started_at": null, "extractors": {}}`.
