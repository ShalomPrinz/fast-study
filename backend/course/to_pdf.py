"""Overview to_pdf phase worker: read one extractor's {slug}-analyzed.md and render
{slug}-analyzed.pdf via the pipeline's convert_to_pdf. A missing .md just skips.
...
def run_to_pdf(course: str, slug: str) -> dict:
    """Render one extractor's analyzed markdown to PDF; write {slug}-analyzed.pdf. Returns a
