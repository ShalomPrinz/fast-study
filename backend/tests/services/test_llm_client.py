...
from services.llm_client import DEFAULT_MODEL, GeminiRateLimitError, LLMClient, parse_gemini_rate_limit


def _quota_body(quota_id: str, quota_value: str = "20", model: str = "gemini-3.5-flash") -> dict:
...
def test_parse_daily_quota():
    info = parse_gemini_rate_limit(_quota_body("GenerateRequestsPerDayPerProjectPerModel-FreeTier"))
...
def test_parse_per_minute_quota():
    body = _quota_body("GenerateRequestsPerMinutePerProjectPerModel-FreeTier", quota_value="15")
...
def test_parse_missing_details_defaults_to_daily():
    info = parse_gemini_rate_limit({"error": {"code": 429, "status": "RESOURCE_EXHAUSTED"}})
    assert info["is_daily"] is True
...
def test_parse_falls_back_to_raw_error_text():
    ...
    text = ("429 RESOURCE_EXHAUSTED. {'error': {'details': [{'violations': "
