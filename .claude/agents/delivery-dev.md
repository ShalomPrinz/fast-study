...
- Published bytes are exactly the bytes the smoke job tested. `publish.yml` never builds, takes no input, and only flips a draft targeted at the dispatched commit; a build step that runs after the smoke job, or a publish path that rebuilds, breaks that.
